...
export default function ChartAnalysis({ className }: ChartAnalysisProps) {
    ...
    return (
        <div ref={containerRef} className={`flex h-full w-full ${className} relative`}>
            ...
            <div style={{ width: leftPanelWidth }} className="flex-shrink-0 h-full overflow-y-auto pr-2 custom-scrollbar">
                <div className="space-y-6 pb-6">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
                        <h2 className="text-lg font-bold mb-4 text-slate-800">{t('chart.input')}</h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.selectFile')}</label>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.delimiter')}</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.encoding')}</label>
                            ...
                            {headers.length > 0 && (
                                <>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.dateColumn')}</label>
                                    ...
                                    {dateColumn && (
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <label htmlFor="enableDate" className="text-sm text-slate-600">{t('chart.enableDateFilter')}</label>
                                            </div>
                                            {enableDateFilter && (
                                                <div className="grid grid-cols-2 gap-2">
                    ...
                    {/* Chart Settings */}
                    <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
                        <h2 className="text-lg font-bold mb-4 text-slate-800">{t('chart.settings')}</h2>
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.chartType')}</label>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.width')}</label>
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.height')}</label>
                            ...
                            {chartType === 'heatmap' && (
                                <>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.valueColumn')}</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.xAxisUnit')}</label>
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.yAxisUnit')}</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.colorTheme')}</label>
                            ...
                            {chartType === 'frequency' && (
                                <>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.analysisColumn')}</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.binSize')}</label>
                            ...
                            {!['heatmap', 'frequency'].includes(chartType) && (
                                <>
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.xAxis')}</label>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.yAxis')}</label>
                                    ...
                                    <div className="space-y-2 pt-2 border-t border-slate-200">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <label htmlFor="enableOutlier" className="text-sm text-slate-600">{t('chart.enableOutlier')}</label>
                                        </div>
                                        {enableOutlierFilter && (
                                            <div>
                                                <label className="block text-sm font-medium text-slate-600 mb-1">{t('chart.iqrMultiplier')}</label>
                            ...
                            <button onClick={generateChart} disabled={loading || csvData.length === 0} className="w-full bg-[#0085CA] hover:bg-[#006699] text-white font-bold py-2 px-4 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                {loading ? t('chart.processing') : t('chart.loadData')}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/* Resizer Handle */}
            <div
                className="w-2 bg-slate-200 hover:bg-[#0085CA] cursor-col-resize flex-shrink-0 transition-colors"
                onMouseDown={startResizing}
            />

            {/* Right Panel: Chart Display */}
            <div className="flex-1 min-w-0 pl-2">
                <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6 h-full min-h-[600px] flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-lg font-bold text-slate-800">{t('chart.display')}</h2>
                        {error && <span className="text-red-500 text-sm">{error}</span>}
                    </div>

                    <div className="flex-1 bg-white rounded-xl p-4 flex items-center justify-center overflow-auto border border-slate-100">
                        {plotData.length > 0 ? (
                            <Plot
                                data={plotData}
                                layout={plotLayout}
